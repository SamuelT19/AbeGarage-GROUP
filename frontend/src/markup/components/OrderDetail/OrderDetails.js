...
import { getStatusClass } from "../../../util/statusUtils"; // Adjust the import path if necessary
...
function OrderDetails() {
  ...
  const { orderId, orderStatus: orderStatusParam, orderHash } = useParams();
  const { isEmployee, isAdmin, isManager } = useAuth();
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        setSingleOrder(response);
        console.log(response);
        setOrderStatus(response.order_status || Number(orderStatusParam));
      } catch (error) {
        console.log(error);
      }
    };
    fetchOrder();
  }, [orderId, orderStatusParam]);
  ...
  const handleSubmitProgress = async () => {
    try {
      ...
      await OrderService.updateOrderProgress(orderId, progress);
      console.log(progress); // Log the data for debugging
      alert("Order progress submitted successfully.");
    } catch (error) {
      console.log(error);
      alert("Failed to submit order progress.");
